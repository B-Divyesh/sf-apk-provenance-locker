...
Open `/?demo=1` or `/demo`. The demo starts with two realistic APK inventory
records: F-Droid and KeePassDX. Their package names, versions, SHA-256 file
fingerprints, and signing-history details are sample metadata, not verification results. Use
...
the two records. **Start for real**, the wordmark, and **Locker** remove the
demo key, demo license keys, and demo file database before opening the real
locker. The app shell and the sample
