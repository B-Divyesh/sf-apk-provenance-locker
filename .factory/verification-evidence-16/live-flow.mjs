...
const kit = await readFile(kitPath);
...
await page.evaluate(async ({ raw }) => {
  ...
  const records = Array.from({ length: 20 }, (_, index) => ({ ...verified, id: `verification-16-${index}`, filename: `lineage-${index}.apk`, added: new Date(Date.now() + index).toISOString(), backup: true }));
  ...
  await new Promise((resolve, reject) => { const transaction = db.transaction('files', 'readwrite'); for (const record of records) transaction.objectStore('files').put(bytes.buffer.slice(0), record.id); transaction.oncomplete = resolve; transaction.onerror = () => reject(transaction.error); });
  db.close();
}, { raw: fixture.toString('base64') });
...
await page.getByRole('button', { name: 'Download encrypted kit' }).click();
const bulkPath = await (await bulkDownloadPromise).path();
const bulkKit = await readFile(bulkPath);
await page.getByRole('button', { name: 'Validate a restore kit' }).click();
...
await page.getByRole('button', { name: 'Check APK evidence' }).click();
await page.getByRole('heading', { name: '20 APKs match' }).waitFor({ timeout: 60_000 });
...
const report = {
  ...
  normal: {
    expectedHash,
    package: evidenceText.includes('android.appsecurity.cts.tinyapp'),
    version: evidenceText.includes('1.0 · code 10'),
    fingerprint: evidenceText.includes(expectedHash),
    signatures: evidenceText.includes('v1 + v2 + v3'),
    history: evidenceText.includes('Verified signing history'),
  },
  passwordBoundary: shortPassword,
  mismatchError,
  wrongPasswordError,
  validation: {
    oneMatch: validationText.includes('1 APK matches'),
    missingSampleCopies: (validationText.match(/saved APK copy missing/g) || []).length,
    verifiedCopyDownload: validationText.includes('Download verified APK'),
    importAvailable: validationText.includes('Import verified records'),
  },
  bulk: { records: 20, report: await page.getByRole('heading', { name: '20 APKs match' }).innerText() },
  requests: { total: requests.length, thirdParty: requests.filter(request => new URL(request.url).origin !== base), withBody: requests.filter(request => request.bodyBytes || request.method !== 'GET') },
  errors,
};
await writeFile('.factory/verification-evidence-16/live-flow.json', `${JSON.stringify(report, null, 2)}\n`);
