...
for(const url of [...links].sort()){
  const response=await context.request.get(url,{maxRedirects:10,timeout:30000});
...
await browser.close();
await writeFile('.factory/verification-evidence-19/link-audit.json',`${JSON.stringify(results,null,2)}\n`);
