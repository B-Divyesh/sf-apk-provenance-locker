...
for(const viewport of [{name:'desktop',width:1280,height:900},{name:'mobile',width:390,height:844}]){
  for(const route of ['/','/demo','/privacy','/terms','/verification-19-missing']){
    ...
    const metrics=await page.evaluate(()=>({
      title:document.title,lang:document.documentElement.lang,h1:document.querySelectorAll('h1').length,main:document.querySelectorAll('main').length,
      ...
      smallTargets:[...document.querySelectorAll('a,button')].filter(element=>{const box=element.getBoundingClientRect();return box.width>0&&box.height>0&&(box.width<44||box.height<44)}).map(element=>({text:element.textContent?.trim(),width:element.getBoundingClientRect().width,height:element.getBoundingClientRect().height})),
    }));
    ...
    report.routes.push({viewport:viewport.name,route,status:response?.status(),...metrics,zoomOverflow,seriousCritical:serious.map(item=>item.id),zoomSeriousCritical:zoomSerious.map(item=>item.id),errors,thirdParty:requests.filter(item=>new URL(item.url).origin!==base)});
    await context.close();
  }
}

{
  ...
  await page.screenshot({path:'.factory/verification-evidence-19/live-cold-mobile.png',fullPage:false});
  await page.getByRole('link',{name:'Try it with sample data'}).click();
  await page.waitForURL(url=>url.pathname==='/demo'||url.searchParams.get('demo')==='1');
  report.firstRead.afterClick={url:page.url(),banner:await page.getByText('Demo — sample data, nothing is saved').isVisible(),records:await page.locator('.record').count(),firstRecordTop:(await page.locator('.record').first().boundingBox())?.y,resetVisible:await page.getByRole('button',{name:'Reset demo'}).isVisible(),startRealVisible:await page.getByRole('link',{name:'Start for real'}).isVisible()};
  await page.screenshot({path:'.factory/verification-evidence-19/live-demo-mobile.png',fullPage:false});
  await context.close();
}

{
  ...
  const skipTarget=await page.evaluate(()=>({hash:location.hash,activeId:document.activeElement?.id,activeText:document.activeElement?.textContent?.trim()}));
  const trigger=page.getByRole('button',{name:/verify an apk/i}).first();
  await trigger.focus();
  await page.keyboard.press('Enter');
  ...
  report.keyboard={firstFocus,focusStyle,skipTarget,dialogOpened,openFocus,dialogClosed:await dialog.count()===0,returnedToTrigger:await trigger.evaluate(element=>element===document.activeElement)};
  await context.close();
}
...

{
  ...
  report.pwa={controlled:await page.evaluate(()=>Boolean(navigator.serviceWorker.controller)),update,cacheNames,offlineRecords:await page.locator('.record').count(),offlineBanner:await page.getByText('Demo — sample data, nothing is saved').isVisible(),errors};
  await context.close();
}

await browser.close();
await writeFile('.factory/verification-evidence-19/live-audit.json',`${JSON.stringify(report,null,2)}\n`);
