...
function readLen(bytes:Uint8Array,p:number){if(p+4>bytes.length)throw new Error('Unexpected end of signing block.');const n=new DataView(bytes.buffer,bytes.byteOffset+p,4).getUint32(0,true);if(n<0||p+4+n>bytes.length)throw new Error('Invalid signing block.');return [bytes.slice(p+4,p+4+n),p+4+n] as const}
/** Validate the ZIP container and require the mandatory Android manifest. */
export function assertApkArchive(data:ArrayBuffer){
 ...
 const entries=view.getUint16(eocd+10,true), cdSize=u32(view,eocd+12), cd=u32(view,eocd+16);
 if(entries<1||cd+cdSize>n)throw new Error('This APK has an invalid ZIP directory.');
 ...
 for(let i=0;i<entries;i++){
   ...
   if(p+46+nameLength+extraLength+commentLength>n)throw new Error('This APK has a truncated ZIP entry.');
...
/** Extracts v2/v3 signing certificate fingerprints from the APK Signing Block. */
export async function readSigner(data:ArrayBuffer):Promise<{schemes:string[]; signer:string}>{
 ...
 const magic='APK Sig Block 42', m=te.encode(magic); let start=cd-24; for(let i=0;i<m.length;i++)if(new Uint8Array(data)[cd-16+i]!==m[i])return {schemes:[],signer:'v1 or unavailable'};
 const size=u64(v,cd-24); const block=cd-(size+8); if(block<0||u64(v,block)!==size)return {schemes:[],signer:'Unreadable'};
 let p=block+8, schemes:string[]=[]; let cert:Uint8Array|undefined;
 while(p<cd-24){ const len=u64(v,p); const id=u32(v,p+8); const value=new Uint8Array(data,p+12,len-4); if(id===0x7109871a||id===0xf05368c0){schemes.push(id===0x7109871a?'v2':'v3');try{const [signer]=readLen(value,0);const [signed]=readLen(signer,0);let q=0;const [,afterDigests]=readLen(signed,q);q=afterDigests;const [certs]=readLen(signed,q);const [first]=readLen(certs,0);cert=first}catch{/* retain scheme evidence */} } p+=8+len; }
 return {schemes,signer:cert?await sha256(cert):schemes.length?'Certificate unreadable':'v1 or unavailable'};
}
