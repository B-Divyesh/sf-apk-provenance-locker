...
describe('deployment and Android release configuration',()=>{
  ...
  it('builds v0.5.0 packages only from the matching tag and byte-checks the packaged web build',()=>{
    ...
    expect(workflow).toContain("grep -q 'android:allowBackup.*0x0' app-manifest.txt");
  ...
  it('keeps reviewed visitor copy concrete and discloses the published Android package',()=>{
    ...
    expect(app).toContain('/releases/download/v0.5.0/app-release.apk');
    expect(app).toContain('/releases/download/v0.5.0/SHA256SUMS');
